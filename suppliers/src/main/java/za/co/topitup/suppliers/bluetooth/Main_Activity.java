...
/*****************************************************************************************************/	
	private void KeyListenerInit() {
		...
		printbmpButton = findViewById(R.id.btn_printpicture);
		printbmpButton.setOnClickListener(this);

		btnScanButton = findViewById(R.id.button_scan);
		btnScanButton.setOnClickListener(this);
		
		hexBox = findViewById(R.id.checkBoxHEX);
		hexBox.setOnClickListener(this);
		
		width_58mm = findViewById(R.id.width_58mm);
		width_58mm.setOnClickListener(this);
		
		width_80 = findViewById(R.id.width_80mm);
		width_80.setOnClickListener(this);
		
		imageViewPicture = findViewById(R.id.imageViewPictureUSB);
		imageViewPicture.setOnClickListener(this);
	
		btnClose = findViewById(R.id.btn_close);
		btnClose.setOnClickListener(this);
		
		btn_BMP = findViewById(R.id.btn_prtbmp);
		btn_BMP.setOnClickListener(this);
		
		btn_ChoseCommand = findViewById(R.id.btn_prtcommand);
		btn_ChoseCommand.setOnClickListener(this);
		
		btn_prtsma = findViewById(R.id.btn_prtsma);
		btn_prtsma.setOnClickListener(this);
		
		btn_prttableButton = findViewById(R.id.btn_prttable);
		btn_prttableButton.setOnClickListener(this);
		
		btn_prtcodeButton = findViewById(R.id.btn_prtbarcode);
		btn_prtcodeButton.setOnClickListener(this);
		
		btn_camer = findViewById(R.id.btn_dyca);
		btn_camer.setOnClickListener(this);
		
		btn_scqrcode = findViewById(R.id.btn_scqr);
		btn_scqrcode.setOnClickListener(this);
		
		Simplified = findViewById(R.id.gbk12);
		Simplified.setOnClickListener(this);
		big5 = findViewById(R.id.big5);
		big5.setOnClickListener(this);
		thai = findViewById(R.id.thai);
		thai.setOnClickListener(this);
		Korean = findViewById(R.id.kor);
	...
	public void onClick(View v) {
		...
		if (v.getId() == R.id.button_scan) {
			...
			if (hexBox.isChecked()) {
				String str = editText.getText().toString().trim(); // Remove leading and trailing spaces
				if (str.length() > 0) {
					str = Other.RemoveChar(str, ' ').toString();
					if (str.length() <= 0) return;
					if ((str.length() % 2) != 0) {
						Toast.makeText(getApplicationContext(), getString(R.string.msg_state), Toast.LENGTH_SHORT).show();
						return;
					}
					byte[] buf = Other.HexStringToBytes(str);
					SendDataByte(buf);
		...
		} else if (v.getId() == R.id.width_58mm || v.getId() == R.id.width_80mm) {
			is58mm = v == width_58mm;
			width_58mm.setChecked(is58mm);
			width_80.setChecked(!is58mm);

		} else if (v.getId() == R.id.btn_printpicture) {
			GraphicalPrint();

		} else if (v.getId() == R.id.imageViewPictureUSB) {
			Intent loadpicture = new Intent(Intent.ACTION_PICK, MediaStore.Images.Media.EXTERNAL_CONTENT_URI);
			startActivityForResult(loadpicture, REQUEST_CHOSE_BMP);

		} else if (v.getId() == R.id.btn_prtbmp) {
			Print_BMP();

		} else if (v.getId() == R.id.btn_prtcommand) {
			CommandTest();

		} else if (v.getId() == R.id.btn_prtsma) {
			SendDataByte(Command.ESC_Init);
			SendDataByte(Command.LF);
			Print_Ex();

		} else if (v.getId() == R.id.btn_prttable) {
			SendDataByte(Command.ESC_Init);
			SendDataByte(Command.LF);
			PrintTable();

		} else if (v.getId() == R.id.btn_prtbarcode) {
			printBarCode();

		} else if (v.getId() == R.id.btn_scqr) {
			createImage();

		} else if (v.getId() == R.id.btn_dyca) {
			dispatchTakePictureIntent(REQUEST_CAMER);

		} else {
			// Default case (do nothing or log)
		}
		...
		public void handleMessage(Message msg) {
			...
			case MESSAGE_STATE_CHANGE:
				...
				case BluetoothService.STATE_CONNECTED:
					...
					SendDataByte(PrinterCommand.POS_Print_Text(msgq, CHINESE, 0, 1, 1, 0));
